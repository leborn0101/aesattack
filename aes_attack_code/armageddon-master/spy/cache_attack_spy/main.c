...
//     offset_update_time);
static void attack_spy_master(Matrix *mtr, size_t offset,
                              useconds_t offset_update_time);
static void attack_slave(libflush_session_t *libflush_session, uint8_t *m,
                         size_t threshold, size_t offset,
                         size_t number_of_tests, bool show_timing,
                         FILE *logfile);
static void printResult(Matrix *mtr, uint64_t *shared_addresses_hits);
...
static void print_help(char *argv[])
{
  ...
  fprintf(stdout,
          "\t-n, -number-of-tests <value>\t Number of tests per address\n");
  fprintf(stdout, "\t-u, -offset-update-time <value>\t Interval in seconds to "
                  "update the offset\n");
...
int main(int argc, char *argv[])
{
  /* Define parameters */
  char *filename = NULL;
  char *matrixfilename = "/data/local/tmp/config.txt";
  size_t offset = 0;
  size_t range = 0;
  size_t threshold = 0;
  ...
  // read matrix config file
  FILE *matrixfile = fopen(matrixfilename, "r");
  if (matrixfile == NULL)
  {
    fprintf(stderr, "Error: can not open matrix config file in path: %s\n",
            matrixfilename);
  }
...
/* Setup shared memory */
  ...
  if (ac == 0)
  {
    fprintf(stderr, "Error：cannot read Address count\n");
    return -1;
  }

  shared_addresses_hits_shm_fd = open("/" ASHMEM_NAME_DEF, O_RDWR);
  if (shared_addresses_hits_shm_fd < 0)
  {
    fprintf(stderr, "Error: Could not create shared memory.\n");
    return -1;
  }
  ioctl(shared_addresses_hits_shm_fd, ASHMEM_SET_NAME, "shared_addresses_hits");
...
// todo: 添加对shared_addresses_hits_shm_fd 的支持
#endif
#endif
...
/* Start master and slaves */
  ...
  for (unsigned int i = 0; i < number_of_forks + 1; i++)
  {
    ...
    else if (pids[i] == 0)
    {
      ...
      if (i == 0)
      {
        ...
        attack_spy_master(mtr, offset, offset_update_time);
      }
      else
      {
  ...
  for (unsigned int i = 0; i < number_of_forks + 1; i++)
  {
    ...
    if (pthread_create(&threads[i], NULL, attack_thread, &thread_data[i]) !=
        0)
    {
      return -1;
    }
  ...
  {
    close(shared_addresses_hits_shm_fd);
  }
#else
  if (shared_data != NULL)
  {
...
// {
//   do {
//     for (shared_data->current_offset = 0; shared_data->current_offset <
//     range; shared_data->current_offset += 64) {
...
static void attack_spy_master(Matrix *mtr, size_t offset,
                              useconds_t offset_update_time)
{
  uint64_t *shared_addresses_hits_record =
