...
#define Te2(x) temporary[x + 512]
#define Te3(x) temporary[x + 768]

#define AES_PRINT_TIME 1
#define MAPPING_SIZE (1024*1024)
...
size_t aes_get_set(void *address) {
  void *phy;
  mem_addr_vir2phy(address, &phy);
  size_t set = get_set_index(phy);
  return set;
}

void aes_ecb_encrypt(const unsigned char *ckey, const unsigned char *pt,
                     unsigned char *ct, aesTableSet *ts) {
  static unsigned int flag = 0;
  static int aesflag = 1;
  uint64_t tmp;
  static void *tsa;
  if (aesflag) {
    tsa = mmap(NULL, MAPPING_SIZE, PROT_READ | PROT_WRITE,
               MAP_POPULATE | MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    temporary = (uint32 *)tsa;
	uint64_t index;
    for (index = 0; index < AES_TABLE_SIZE; index++) {
      temporary[index] = TE0[index];
    }
    for (; index < 2 * AES_TABLE_SIZE; index++) {
      temporary[index] = TE1[index - AES_TABLE_SIZE];
    }
    for (; index < 3 * AES_TABLE_SIZE; index++) {
      temporary[index] = TE2[index - 2 * AES_TABLE_SIZE];
    }
    for (; index < 4 * AES_TABLE_SIZE; index++) {
      temporary[index] = TE3[index - 3 * AES_TABLE_SIZE];
    }
	for (int i = 0; i < AES_TABLE_SIZE * 4; i++) {
		LOGI("%5d", aes_get_set(&temporary[i]));
	}
	for (int i = 0; i < AES_TABLE_SIZE * 4; i++) {
		LOGI("%x", temporary[i]);
	}
  ...
  if (aesflag) {
    for (int i = 0; i < 256; i++) {
      tmp += (Te0(i) + Te1(i) + Te2(i) + Te3(i));
    }
    LOGI("tmp = %llu", tmp);
  } else if (ckey == NULL) {
    for (int i = 0; i < 256; i++) {
      if (ts->t0_si[i] != aes_get_set(&Te0(i)) ||
          ts->t1_si[i] != aes_get_set(&Te1(i)) ||
          ts->t2_si[i] != aes_get_set(&Te2(i)) ||
          ts->t3_si[i] != aes_get_set(&Te3(i))) {
        LOGI("WARNING : table change location!!!!");
      }
    }
    LOGI("location check finished!!!!");
    return;
  }

#ifdef AESDEBUG
  FILE *aesfp;
  if ((aesfp = fopen("sdcard/aeslog", "a")) == NULL) {
    LOGI("ERROE : could not open file aeslog!!");
    return;
  }
  fprintf(aesfp, "k :");
  for (int i = 0; i < 16; i++) {
    fprintf(aesfp, " %d", ckey[i]);
  }
  fprintf(aesfp, "\nv :");
  for (int i = 0; i < 16; i++) {
    fprintf(aesfp, " %d", pt[i]);
  }
  fprintf(aesfp, "\n");
#endif

  uint32_t s0, s1, s2, s3, t0, t1, t2, t3, *rk, temp;
  int Nr, r, i;
  psAesKey_t key;
...
#ifdef AESDEBUG
  fprintf(aesfp, "t0 map table : ");
  for (int i = 0; i < 256; i++) {
    fprintf(aesfp, "%10d", aes_get_set(&Te0(i)));
  }
  fprintf(aesfp, "\n");
  fprintf(aesfp, "t1 map table : ");
  for (int i = 0; i < 256; i++) {
    fprintf(aesfp, "%10d", aes_get_set(&Te1(i)));
  }
  fprintf(aesfp, "\n");
  fprintf(aesfp, "t2 map table : ");
  for (int i = 0; i < 256; i++) {
    fprintf(aesfp, "%10d", aes_get_set(&Te2(i)));
  }
  fprintf(aesfp, "\n");
  fprintf(aesfp, "t3 map table : ");
  for (int i = 0; i < 256; i++) {
    fprintf(aesfp, "%10d", aes_get_set(&Te3(i)));
  }
  fprintf(aesfp, "\n");
  fprintf(aesfp, "aes access t0 : %d %d %d %d\n",
          aes_get_set(&Te0(byte(s0, 3))), aes_get_set(&Te0(byte(s1, 3))),
          aes_get_set(&Te0(byte(s2, 3))), aes_get_set(&Te0(byte(s3, 3))));
  fprintf(aesfp, "aes access t1 : %d %d %d %d\n",
          aes_get_set(&Te1(byte(s1, 2))), aes_get_set(&Te1(byte(s2, 2))),
          aes_get_set(&Te1(byte(s3, 2))), aes_get_set(&Te1(byte(s0, 2))));
  fprintf(aesfp, "aes access t2 : %d %d %d %d\n",
          aes_get_set(&Te2(byte(s2, 1))), aes_get_set(&Te2(byte(s3, 1))),
          aes_get_set(&Te2(byte(s0, 1))), aes_get_set(&Te2(byte(s1, 1))));
  fprintf(aesfp, "aes access t3 : %d %d %d %d\n",
          aes_get_set(&Te3(byte(s3, 0))), aes_get_set(&Te3(byte(s0, 0))),
          aes_get_set(&Te3(byte(s1, 0))), aes_get_set(&Te3(byte(s2, 0))));
#endif
