...
void aes_ecb_encrypt(const unsigned char *ckey, const unsigned char *pt,
  ...
  if (aesflag) {
    ...
    tm = fopen("tablemap", "w");
    if (tm == NULL) {
      fprintf(stderr, "could not open file tablemap!!\n");
      return;
    }

	for (int i = 0; i < AES_TABLE_SIZE * 4; i++) {
      if (i % 16 == 0)  fprintf(tm, "\n");
		  fprintf(tm, "%5d", aes_get_set(&temporary[i]));
	  }
    fprintf(tm, "\n");
    fflush(tm);
    fclose(tm);
  ...
  if (aesflag) {
    for (int i = 0; i < 256; i++) {
      if (aes_get_set(&Te0(i)) != aes_get_set(&Te0((i / 16) * 16))) {
        fprintf(stderr, "table map error!!\n");
      }
      if ((aes_get_set(&Te0(0)) + i / 16) != aes_get_set(&Te0(i))) {
        fprintf(stderr, "table map error!!!\n");
      }
    }
  } else if (ckey == NULL) {
    for (int i = 0; i < 256; i++) {
      if (ts->t0_si[i] != aes_get_set(&Te0(i)) ||
        ...
        fprintf(stderr, "WARNING : table change location!!!!\n");
      }
    }
    fprintf(stdout, "location check finished!!!!\n");
    return;
  }

  uint32_t t0, t1, t2, t3;

  t0 = Te0(ckey[0] ^ pt[0]) ^ Te1(ckey[5] ^ pt[5]) ^ Te2(ckey[10] ^ pt[10]) ^
      Te3(ckey[15] ^ pt[15]);

  t1 = Te0(ckey[4] ^ pt[4]) ^ Te1(ckey[9] ^ pt[9]) ^ Te2(ckey[14] ^ pt[14]) ^
      Te3(ckey[3] ^ pt[3]);

  t2 = Te0(ckey[8] ^ pt[8]) ^ Te1(ckey[13] ^ pt[13]) ^ Te2(ckey[2] ^ pt[2]) ^
      Te3(ckey[7] ^ pt[7]);

  t3 = Te0(ckey[12] ^ pt[12]) ^ Te1(ckey[1] ^ pt[1]) ^ Te2(ckey[6] ^ pt[6]) ^
      Te3(ckey[11] ^ pt[11]);
      
/*
  t0 = Te0(ckey[0] ^ pt[0]) ^ Te0((ckey[5] ^ pt[5]) + 256)  ^ Te0((ckey[10] ^ pt[10]) + 512) ^
      Te0((ckey[15] ^ pt[15]) + 768);

  t1 = Te0(ckey[4] ^ pt[4]) ^ Te0((ckey[9] ^ pt[9]) + 256) ^ Te0((ckey[14] ^ pt[14]) + 512) ^
      Te0((ckey[3] ^ pt[3]) + 768);

  t2 = Te0(ckey[8] ^ pt[8]) ^ Te0((ckey[13] ^ pt[13]) + 256) ^ Te0((ckey[2] ^ pt[2]) + 512) ^
      Te0((ckey[7] ^ pt[7]) + 768);

  t3 = Te0(ckey[12] ^ pt[12]) ^ Te0((ckey[1] ^ pt[1]) + 256) ^ Te0((ckey[6] ^ pt[6]) + 512) ^
      Te0((ckey[11] ^ pt[11]) + 768);
  int t = t0 + t1 + t2 + t3;
  */
  if (aesflag) {
    for (int i = 0; i < 256; i++) {
