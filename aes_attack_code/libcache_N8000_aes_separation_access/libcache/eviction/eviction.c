...
typedef struct libcache_eviction_s {
...
void find_congruent_addresses(libcache_session_t* session, libcache_eviction_t*
    eviction, size_t index, uintptr_t physical_address);
void find_congruent_addresses_ex(libcache_session_t* session, libcache_eviction_t* eviction);

void
libcache_eviction_evict(libcache_session_t* session, void* address)
{
  libcache_eviction_t* eviction = (libcache_eviction_t*) session->data;
  ...
  uintptr_t physical_address = libcache_get_physical_address(session, (size_t) address);
...
bool
libcache_eviction_init(libcache_session_t* session, libcache_session_args_t* args)
{
  if (session == NULL) {
    return false;
  }
  ...
  libcache_eviction_t* eviction = calloc(1, sizeof(libcache_eviction_t));
  if (eviction == NULL) {
    return false;
  }
  ...
  pthread_mutex_unlock(&(eviction->memory.lock));
	...
	find_congruent_addresses_ex(session, eviction);

  return true;
}

bool
libcache_eviction_terminate(libcache_session_t* session)
{
  if (session == NULL) {
    return false;
  }
...
void
libcache_eviction_prime(libcache_session_t* session, size_t set_index)
{
  libcache_eviction_t* eviction = (libcache_eviction_t*) session->data;
...
{
  //uint64_t access_mem_time[ADDRESS_COUNT];
  libcache_eviction_t* eviction = (libcache_eviction_t*) session->data;
  ...
  uint64_t start = libcache_get_timing_start(session);
  for (int i = ADDRESS_COUNT - 1; i >= 0; i -= 1) {
    libcache_access_memory(congruent_address_cache_entry.congruent_virtual_addresses[i]);
  }
  uint64_t end = libcache_get_timing_end(session);
  /*uint64_t start, end;
  uint64_t base_time;
  get_current_basetime(session, &base_time);
  for (int i = ADDRESS_COUNT + 20; i >= 0; i -= 1) {
    if (i > ADDRESS_COUNT - 1) {
      ...
      access_mem_time[i] = end - start;
    }
  }
  uint64_t probe_time = 0;
  for (int j = 0; j < ADDRESS_COUNT; j++) {
    probe_time += access_mem_time[j];
  }
  FILE* fp;
  if ((fp = fopen("/sdcard/probetime", "a")) == NULL) {
    LOGI("Error : could not open file /sdcard/probetime!!");
    return -1;
  }
  fprintf(fp, "base_time : %4llu\n", base_time);
  for (int i = ADDRESS_COUNT - 1; i >= 0; i--) {
    if (access_mem_time[i] > (session->basetime)) {
      fprintf(fp, "%10llu", access_mem_time[i] - (session->basetime));
    } else {
      fprintf(fp, "%10d", 0);
    }
  }
  ...
  pthread_mutex_unlock(&(eviction->congruent_address_cache[set_index].lock));
#endif
  /*if (probe_time > ((session->basetime) * ADDRESS_COUNT))
    return probe_time - ((session->basetime) * ADDRESS_COUNT);*/
  return end - start;

}

size_t
libcache_eviction_get_number_of_sets(libcache_session_t* session)
{
  (void) session;
...
size_t
libcache_eviction_get_set_index(libcache_session_t* session, void* address)
{
  uintptr_t physical_address = libcache_get_physical_address(session, (uintptr_t) address);
  ...
  for (unsigned int i = 0; i < ES_EVICTION_COUNTER; i += 1) {
    for (unsigned int j = 0; j < ES_NUMBER_OF_ACCESSES_IN_LOOP; j++) {
      for (unsigned int k = 0; k < ES_DIFFERENT_ADDRESSES_IN_LOOP; k++) {
        libcache_access_memory(congruent_address_cache_entry->congruent_virtual_addresses[i+k]);
      }
    }
  }
#ifdef PTHREAD_ENABLE
...
inline void
find_congruent_addresses(libcache_session_t* session, libcache_eviction_t*
    eviction, size_t index, uintptr_t physical_address)
{
	LOGI("oooooh fuck i should not be here!!!!!");
  ...
  for (unsigned int i = 0; i < eviction->memory.mapping_size; i += LINE_LENGTH) {
    ...
    pthread_mutex_unlock(&(eviction->memory.lock));
#endif
    uintptr_t physical_address_2 = libcache_get_physical_address(session, (uintptr_t) virtual_address_2);
...
inline void
find_congruent_addresses_ex(libcache_session_t* session, libcache_eviction_t* eviction) {
  unsigned int found_sum = 0;
  unsigned int found[NUMBER_OF_SETS];
  memset(found, 0, NUMBER_OF_SETS * sizeof(unsigned int));
  for (unsigned int i = 0; i < eviction->memory.mapping_size; i += LINE_LENGTH) {
#ifdef PTHREAD_ENABLE
    pthread_mutex_lock(&(eviction->memory.lock));
#endif
    ...
    uintptr_t physical_address_2 = libcache_get_physical_address(session, (uintptr_t) virtual_address_2);
    uint64_t index_2 = (physical_address_2 >> LINE_LENGTH_LOG2) % NUMBER_OF_SETS;
    if (found[index_2] < ADDRESS_COUNT) {
      eviction->congruent_address_cache[index_2].congruent_virtual_addresses[(found[index_2])] = virtual_address_2;
      found[index_2] += 1;
      found_sum++;
      if (found[index_2] == ADDRESS_COUNT)
        eviction->congruent_address_cache[index_2].used = true;
    }	
    if (found_sum == ADDRESS_COUNT * NUMBER_OF_SETS) {
      break;
    }
  }
  ...
  // Abort if we were not able to find enough addresses
  if (found_sum != ADDRESS_COUNT * NUMBER_OF_SETS)
    LOGI("had not found_enough congruent_addresses_when ex finished!!");	
  for (unsigned int i = 0; i < NUMBER_OF_SETS; i++) {
    if (found[i] != ADDRESS_COUNT)
      LOGI("found[%d] = %d", i, found[i]);
  }
  LOGI("find_congruent_addresses_ex finished!!");	
}	
