...
inline bool
perf_init(libcache_session_t* session, libcache_session_args_t* args)
{
  (void) session;
  (void) args;
...
inline bool
perf_terminate(libcache_session_t* session)
{
...
inline uint64_t
perf_get_timing(libcache_session_t* session)
{
  long long result = 0;
...
inline void
perf_reset_timing(libcache_session_t* session)
{
...
inline bool
thread_counter_init(libcache_session_t* session, libcache_session_args_t* args)
{
  if (session == NULL) {
    return false;
  }
...
inline bool
thread_counter_terminate(libcache_session_t* session)
{
  if (session == NULL) {
    return false;
  }
...
inline uint64_t
thread_counter_get_timing(libcache_session_t* session)
{
  libcache_memory_barrier(session);
...
thread_counter_func(void* data) {
  ...
  libcache_session_t* session = thread_data->session;
